<script setup>
import { ChartCrosshair, ChartLegend, defaultColors } from '@common/ui/chart'
...
<template>
  <div :class="cn('w-full h-auto flex flex-col items-end', $attrs.class ?? '')">

    <VisXYContainer :data="data" :style="{ height: isMounted ? '340px' : 'auto' }" :margin="margin">
      <ChartCrosshair
        v-if="showTooltip"
        :colors="colors"
        :items="legendItems"
      ...
      <VisAxis
        v-if="showYAxis"
        type="y"
        :tick-values="[0, 250, 500, 750, 1000]"
        :tick-line="false"
